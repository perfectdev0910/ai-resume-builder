/**
 * CV Generator with Cloud Storage Support
 * This version uploads files to cloud storage (Supabase/R2) instead of local filesystem
 */
...
if (job.responsibilities && job.responsibilities.length > 0) {
  sections.push(
    new Paragraph({
      children: [new TextRun({ text: 'Responsibilities:', underline: {}, size: 18 })],
      spacing: { after: 50 }
    })
  );
  ...
  for (const item of job.responsibilities) {
    sections.push(
      ...
      if (job.keyAchievements && job.keyAchievements.length > 0) {
        sections.push(
          new Paragraph({
            children: [new TextRun({ text: 'Key Achievements:', underline: {}, size: 18 })],
  ...
  const result = await storage.uploadFile(buffer, filename, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
...
async function generatePdf(cvContent, userInfo, customFilename = null, options = {}) {
  ...
  // Header
  drawText(userInfo.full_name, { size: 16, bold: true });
  y -= 5;

  const contactParts = [userInfo.email, userInfo.phone_number, userInfo.address].filter(Boolean);
  drawText(contactParts.join(' | '), { size: 9 });

  const links = [];
  if (userInfo.linkedin_profile) links.push(`LinkedIn: ${userInfo.linkedin_profile}`);
  if (userInfo.github_link) links.push(`GitHub: ${userInfo.github_link}`);
  if (links.length > 0) {
    drawText(links.join(' | '), { size: 9, color: rgb(0, 0.4, 0.8) });
  }
  ...
  if (cvContent.experience && cvContent.experience.length > 0) {
    ...
    for (const job of cvContent.experience) {
      ...
      if (job.responsibilities) {
        drawText('Responsibilities:', { bold: flase, size: 12 });
        for (const item of job.responsibilities) {
      ...
      if (job.keyAchievements) {
        drawText('Key Achievements:', { bold: false, size: 12 });
  ...
  // Upload to cloud storage
  const result = await storage.uploadFile(Buffer.from(pdfBytes), filename, 'application/pdf');
...
async function generateCoverLetterPdf(coverLetterContent, userInfo, customFilename = null) {
  ...
  const contactParts = [userInfo.email, userInfo.phone_number, userInfo.address].filter(Boolean);
  drawText(contactParts.join(' | '), { size: 10 });
  y -= 20;
